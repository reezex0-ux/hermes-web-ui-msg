...
export function AppShell() {
  ...
  return <div className={`app-shell ${navigationCollapsed ? "navigation-collapsed" : ""}`}>
    ...
    <aside className={`navigation ${navigationOpen ? "open" : ""}`}>
      <div className="navigation-head"><img className="brand-logo" src="./myosotis-logo.png" alt="Myosotis logo" /><div><span className="eyebrow">Personal agent</span><strong>Hermes</strong></div><button className="collapse" type="button" onClick={() => setNavigationCollapsed(value => !value)} aria-label={navigationCollapsed ? "Expand sidebar" : "Collapse sidebar"}>{navigationCollapsed ? "+" : "-"}</button><button className="drawer-close" type="button" onClick={() => setNavigationOpen(false)} aria-label="Close sessions">Close</button></div>
      <Navigation workspace={workspace} adapter={settingsAdapter} activePanel={activePanel} activeProfile={activeProfile} activeSession={activeSession} expandedProfiles={expandedProfiles} live={live} creatingSession={creatingSession} onCreateSession={createSession} onOpenCron={() => { setActivePanel("cron"); setNavigationOpen(false); }} onOpenSettings={() => { setSettingsOpen(true); setNavigationOpen(false); }} onProfile={toggleProfile} onSession={selectSession} />
    </aside>
...
function Navigation({ workspace, adapter, activePanel, activeProfile, activeSession, expandedProfiles, live, creatingSession, onCreateSession, onOpenCron, onOpenSettings, onProfile, onSession }: { workspace: HermesWorkspaceSnapshot; adapter: SameOriginHermesAdapter | null; activePanel: "chat" | "cron"; activeProfile: string; activeSession: string; expandedProfiles: string[]; live: boolean; creatingSession: boolean; onCreateSession: () => void; onOpenCron: () => void; onOpenSettings: () => void; onProfile: (id: string) => void; onSession: (profileId: string, sessionId: string) => void }) {
